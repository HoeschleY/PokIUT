...
# --- ### NOUVEAU ### : Écran de combat placeholder ---
def ecran_combat(game_master: GameMaster):
    """Écran placeholder pour le combat."""
    print(f"Combat lancé avec l'équipe : {[p.name for p in equipe_combat]}")
    en_combat = True
    horloge = pygame.time.Clock()

    texte_combat = police_titre.render("COMBAT EN COURS", True, ROUGE_BOUTON)
    texte_combat_rect = texte_combat.get_rect(center=(LARGEUR // 2, HAUTEUR // 2))
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    if bouton_retour_rect.collidepoint(event.pos):
                        en_combat = False # Quitte l'écran de combat

        FENETRE.fill(NOIR) # Fond noir pour le combat
        FENETRE.blit(texte_combat, texte_combat_rect)
        dessiner_bouton("Retour", bouton_retour_rect, ROUGE_BOUTON, BLANC, police_bouton, pos_souris)
        pygame.display.flip()
        horloge.tick(60)
